...
async function aktifkanNotifikasiHP(userData) {
    ...
    if (!messaging) {
        console.warn("Fitur Push Notification tidak didukung di tab ini.");
        return; 
    }
    ...
    try {
        ...
        if (permission === 'granted') {
            ...
            try {
                const tokenOpts = registration ? { serviceWorkerRegistration: registration } : {};
                currentToken = await getToken(messaging, tokenOpts);
            } catch (getTokenErr) {
                console.warn("Catatan FCM getToken (memerlukan VAPID Key terkonfigurasi di Firebase Console):", getTokenErr.message);
            }

            if (currentToken && userData && userData.username) {
                await fsUpdate(COL.USERS, userData.username, {
                    fcm_token: currentToken
                }).catch(e => console.warn(e));
                
                const targetId = userData.nik || userData.id;
                if (targetId) {
                    try {
                        await updateDoc(doc(db, COL.MASTER_KARYAWAN, String(targetId)), {
                            fcm_token: currentToken
                        });
                    } catch(e) {
        ...
        console.warn('Gagal mengaktifkan notifikasi:', error);
    }
}
